...
task.spawn(function()
    ...
    local function getEquippedTool()
        ...
        if #backpack:GetChildren() == 0 then
            -- Backpack is empty, so a tool is equipped
            if HasFirebrand then return "Firebrand" end
            if HasSword then return "Sword" end
        else
            -- Check what's in backpack
            if backpack:FindFirstChild("ClassicSword") then
                return "ClassicSword" -- Farming tool is in backpack, so farming tool is equipped
            end
            if backpack:FindFirstChild("Firebrand") then
                return "Firebrand" -- Firebrand is in backpack
            end
            if backpack:FindFirstChild("Sword") then
                return "Sword" -- Sword is in backpack
            end
        end
        return nil
    end

    -- Equip the best weapon for slime killing
    local function equipBestWeapon()
        if HasFirebrand then
            -- Firebrand is better, equip it
            pcall(function()
                local args = {[1] = {["Name"] = "Firebrand"}}
    ...
    -- Equip farming tool (ClassicSword)
    local function equipFarmingTool()
        pcall(function()
            local args = {[1] = {["Name"] = "ClassicSword"}}
            game:GetService("ReplicatedStorage").Events.PlayerActivesCommand:FireServer(unpack(args))
        end)
        task.wait(0.3)
    end
            ...
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character:FindFirstChild("Humanoid") then
                ...
                if not TargetSlimeBlob and not collectingTokensNow then
                    if isSquareFarming then
                        ...
                        equipBestWeapon() -- Switch to best weapon if we were farming
                    end
                    ...
                    if not TargetSlimeBlob then
                        if Settings.FarmPollen and CurrentRound >= 0 and CurrentRound <= 6 then
                            ...
                            if not FarmingToolEquipped then
                                equipFarmingTool() -- Equip ClassicSword farming tool
                                FarmingToolEquipped = true -- Mark as equipped so we don't do it again
                            end
                            
                            -- Check before each movement if we have the right tool
                            if getEquippedTool() == "Sword" or getEquippedTool() == "Firebrand" then
                                -- We have a weapon equipped, switch to farming tool
                                equipFarmingTool()
                            end
            ...
            if lastToggleState then
                ...
                pcall(function()
                    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
                        ...
                        -- Equip best weapon when feature is disabled
                        task.wait(0.2)
                        equipBestWeapon()
